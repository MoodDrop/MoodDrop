...
import { ArrowLeft, PenLine, Mic, Square, Play, Trash2, X } from "lucide-react";
...
export default function DropItPage() {
  ...
  const saveTextDropLocally = (trimmedText: string) => {
  ...
  const finalizeAfterTextSave = (destination: "vault" | "gallery") => {
    setDraftText("");
    handleAffirmation();
  ...
  const handleSendToVault = async () => {
    if (!draftText.trim() || !selectedMood) return;

    try {
      setIsSubmittingText(true);
      const trimmedText = draftText.trim();
      saveTextDropLocally(trimmedText);
      finalizeAfterTextSave("vault");
    } catch (error) {
      console.error("Error saving drop to vault:", error);
  ...
  const handleShareToGallery = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("Error sharing drop to Living Gallery:", error);
        alert("Your drop was saved privately, but it could not be shared to the Living Gallery.");
        finalizeAfterTextSave("vault");
        return;
      }

      finalizeAfterTextSave("gallery");
    } catch (error) {
      console.error("Error sharing drop:", error);
      alert("Failed to share your drop. Please try again.");
    } finally {
      setIsSubmittingText(false);
    }
  };
    ...
    <>
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            <div>
              ...
              <button
                type="button"
                onClick={() => {
                  console.log("[MoodDrop] opening destination modal");
              ...
              >
                ...
                <span>{isSubmittingText ? "Dropping..." : "Drop It"}</span>
              </button>
            </div>
      ...
      {showDestinationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[28px] border border-blush-100 bg-white/90 p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="rounded-full p-2 text-warm-gray-500 hover:bg-blush-50"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="mt-6 space-y-3">
              <button
                ...
                className="w-full rounded-2xl border border-blush-100 bg-white px-4 py-4 text-left hover:bg-blush-50 transition"
              >
                <div className="text-sm font-semibold text-warm-gray-800">
                  Send to Echo Vault
                </div>
              ...
              >
                ...
                <div className="mt-1 text-[11px] text-warm-gray-400">
            ...
            <button
              type="button"
              onClick={() => setShowDestinationModal(false)}
              className="mt-4 w-full rounded-2xl bg-cream-100 px-4 py-3 text-sm text-warm-gray-700 hover:bg-cream-200 transition"
            >
              Cancel
            </button>
          </div>
